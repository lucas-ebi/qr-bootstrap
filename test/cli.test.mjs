...
import { PROTOCOL, Receiver, loadKey } from '../fountain.js';
...
test('sign uses QR_BOOTSTRAP_SIGNING_KEY (as in CI), and the frames decode under the public key', async () => {
  const { stdout: key } = await node(['keygen', '-']);
  const publicKey = JSON.parse(key).x;
  const { stdout, stderr: log } = await node(['sign', snake, '--id', 'snake', '--version', '7'], { QR_BOOTSTRAP_SIGNING_KEY: key });
...
test('sign with no key anywhere fails instead of signing with something else', async () => {
  const cwd = await mkdtemp(join(tmpdir(), 'qrboot-cli-'));
  await assert.rejects(node(['sign', snake, '--id', 'snake'], { QR_BOOTSTRAP_SIGNING_KEY: '' }, cwd), /ENOENT|signing-key/);
});
...
test('sign --gif writes a standalone looping GIF with a countdown to --url', async () => {
  ...
  const { stderr } = await node(['sign', snake, '--id', 'snake', '--gif', join(dir, 's.gif'), '--url', url, '--intro', '3', '--scale', '5'], { QR_BOOTSTRAP_SIGNING_KEY: key });
  ...
  assert.equal(readQr(gif.frames[0].pixels, gif.width, gif.height), url + '#scan', 'the countdown opens the loader on its scanner');
...
test('--gif options are validated before anything is written', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'qrboot-cli-'));
  const { stdout: key } = await node(['keygen', '-']);
  const sign = extra => node(['sign', snake, '--id', 'snake', '--gif', join(dir, 'x.gif'), ...extra], { QR_BOOTSTRAP_SIGNING_KEY: key });
